...
import 'package:pokedex/classes/pokemon_type_color.dart';
...
String capitalizeWords(String phrase, String regex, {String? replace}) {
  return phrase.splitMapJoin(RegExp(regex),
      onMatch: (p0) => replace ?? p0.group(0) ?? "",
      onNonMatch: (p1) => toBeginningOfSentenceCase(p1) ?? "");
}

Future<Map<MultiplierType, PokemonMultiplier>> calculateMultipliers(
  ...
  for (dynamic item in types) {
    Map<String, dynamic> typeData =
        await Network.getTypeDetails(item["type"]["url"].toString());
  ...
  attackMultipliers =
      SplayTreeMap.from(attackMultipliers, (a, b) => a.compareTo(b));
  defenseMultipliers =
      SplayTreeMap.from(defenseMultipliers, (a, b) => a.compareTo(b));

  return {
    MultiplierType.attack:
        PokemonMultiplier(MultiplierType.attack, Map.from(attackMultipliers)),
    MultiplierType.defense:
        PokemonMultiplier(MultiplierType.defense, Map.from(defenseMultipliers))
  };
}
