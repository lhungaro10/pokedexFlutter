...
class Network {
  ...
  static Future<List<Pokemon>> getFavoritePokemonList(
      List<String> favorites, int page) async {
    int count = page * 15;
  ...
  static Future<Pokemon> getPokemon(String pokemonUrl) async {
    http.Response result = await http.get(Uri.parse(pokemonUrl));

    Map<String, dynamic> pokemonInfo = jsonDecode(result.body);

    List<PokemonType> types = List<PokemonType>.from(pokemonInfo["types"]
    ...
    List<String> pokemonAbilities = pokemonInfo["abilities"]
        .map<String>((e) =>
            capitalizeWords(e['ability']['name'].toString(), "-", replace: " "))
        .toList();

    List<PokemonStat> pokemonStats = pokemonInfo["stats"]
        .map<PokemonStat>((e) => PokemonStat(e['stat']['name'], e['base_stat']))
        .toList();
  ...
  static Future<String> getGeneration(int pokemonId) async {
    ...
    String genName = specieInfo["generation"]["name"].toString().splitMapJoin(
        RegExp(r"generation-"),
        onMatch: (p0) => capitalizeWords(p0.group(0) ?? "", "-", replace: " "),
        onNonMatch: (p1) => p1.toUpperCase());
    int genIndex =
        int.parse(extractIdFromURL(specieInfo["generation"]["url"]) ?? "0");
  ...
  static Future<Evolution> getEvolutionChain(int pokemonId) async {
    ...
    http.Response resultChain =
        await http.get(Uri.parse(pokemonInfo["evolution_chain"]["url"]));

    Map<String, dynamic> evoChain = jsonDecode(resultChain.body);
    String basePokemonId =
        extractIdFromURL(evoChain["chain"]["species"]["url"]) ?? "0";

    Evolution baseEvo = Evolution(
        int.parse(basePokemonId),
        ["Base"],
        capitalizeWords(evoChain["chain"]["species"]["name"].toString(), "-",
            replace: " "));
  ...
  static List<Evolution> getNextEvolution(List<dynamic> evoList) {
    ...
    for (Map<String, dynamic> evolution in evoList) {
      String basePokemonId =
          extractIdFromURL(evolution["species"]["url"]) ?? "0";
      List<String> trigger = evolution["evolution_details"]
          .map<String>((e) => capitalizeWords(
